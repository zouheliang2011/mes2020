 ...
 */
@RestController
@RequestMapping("/procedure_cfg")
public class ProcedureCfgController {

    @Autowired
    private ProcedureCfgServiceImpl service;


    @ApiOperation(value="查询接口", notes="查询满足条件的数据")
    @GetMapping("/{id}")
    public ResponseBean GetItem(@PathVariable int id){
        ProcedureCfg match =  service.getById(id);
    ...
    @ApiOperation(value="查询接口", notes="查询满足条件的数据")
    @GetMapping("/list")
    public ResponseBean GetItemList(ProcedureCfg entity){
        QueryWrapper<ProcedureCfg> queryWrapper = new QueryWrapper<>();
        ...
        List<ProcedureCfg> list =  service.list(queryWrapper);
    ...
    @PostMapping
    public ResponseBean InsertItem(@RequestBody ProcedureCfg entity) {
    ...
    @PutMapping
    public ResponseBean UpdateItem(@RequestBody ProcedureCfg entity) {
