...
import com.example.entity.SewType;
import com.example.service.impl.ProduceRecordsServiceImpl;
import com.example.service.impl.SewTypeServiceImpl;
 ...
 */
@RestController
@RequestMapping("/sew-type")
public class SewTypeController {
    @Autowired
    private SewTypeServiceImpl service;


    @ApiOperation(value="查询接口", notes="查询满足条件的数据")
    @GetMapping("/{id}")
    public ResponseBean GetItem(@PathVariable int id){
        SewType match =  service.getById(id);
    ...
    @ApiOperation(value="查询接口", notes="查询满足条件的数据")
    @GetMapping("/list")
    public ResponseBean GetItemList(SewType entity){
        QueryWrapper<SewType> queryWrapper = new QueryWrapper<>();

        List<SewType> list =  service.list(queryWrapper);
    ...
    @PostMapping
    public ResponseBean InsertItem(@RequestBody SewType entity) {
    ...
    @PutMapping
    public ResponseBean UpdateItem(@RequestBody SewType entity) {
